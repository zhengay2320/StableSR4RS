...
def save_model_card(
    ...
    inference_note = (
        ...
        "transformer to bf16 (`from sdnq.quantizer import dequantize_sdnq_model`), or load a non-quantized base.\n"
        if quant_training == "FP8 SDNQ"
        else ""
    )
    model_description = f"""
...
def main(args):
    ...
    # Detect an SDNQ-quantized base (fp8 or 4bit); its backend must be imported before the checkpoint can
    # load. With --do_fp8_training we train it in-place in fp8, otherwise we dequantize it to bf16 (below).
    ...
    if fp8_is_sdnq:
        ...
        try:
            ...
            raise ImportError(
                "Loading an SDNQ-quantized Ideogram 4 checkpoint requires the sdnq library: `pip install sdnq`."
            )
    ...
    if args.do_fp8_training and fp8_is_sdnq:
        ...
        # No --do_fp8_training: dequantize the SDNQ base to bf16 (it's inference-only) and train a bf16 LoRA.
        from sdnq.quantizer import dequantize_sdnq_model

        transformer = dequantize_sdnq_model(transformer)
    if args.bnb_quantization_config_path is not None:
    ...
    # Holds the most recent interim validation batch; reused for the model-card gallery when final
    # inference is skipped (e.g. --skip_final_inference).
    images = []
    ...
    if accelerator.is_main_process:
        ...
        # Keep the last interim validation batch (in `images`) for the gallery; final inference overrides it.
